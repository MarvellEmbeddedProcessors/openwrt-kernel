			...
			struct device_attribute *attr, char *buf)
{
	...
	if (!strcmp(name, "regs")) {
		mvPp2PlcrHwRegs(sysfs_cur_hw);
	} else	if (!strcmp(name, "dump")) {
		mvPp2PlcrHwDumpAll(sysfs_cur_hw);
	} else {
			...
			struct device_attribute *attr, const char *buf, size_t len)
{
	...
	if (!strcmp(name, "dump")) {
		mvPp2PlcrHwDumpSingle(sysfs_cur_hw, p);
	} else if (!strcmp(name, "v1_tb_dump")) {
		mvPp2V1PlcrTbCntDump(sysfs_cur_hw, p);
	} else	if (!strcmp(name, "period")) {
		mvPp2PlcrHwBasePeriodSet(sysfs_cur_hw, p);
	} else	if (!strcmp(name, "rate")) {
		mvPp2PlcrHwBaseRateGenEnable(sysfs_cur_hw, p);
	} else	if (!strcmp(name, "min_pkt")) {
		mvPp2PlcrHwMinPktLen(sysfs_cur_hw, p);
	} else	if (!strcmp(name, "edrop")) {
		mvPp2PlcrHwEarlyDropSet(sysfs_cur_hw, p);
	} else	if (!strcmp(name, "enable")) {
		mvPp2PlcrHwEnable(sysfs_cur_hw, p, i);
	/* only for ppv2.1 */
	} else	if (!strcmp(name, "mode")) {
		mvPp2PlcrHwMode(sysfs_cur_hw, p);
	} else  if (!strcmp(name, "color")) {
		mvPp2PlcrHwColorModeSet(sysfs_cur_hw, p, i);
	} else	if (!strcmp(name, "config")) {
		mvPp2PlcrHwTokenConfig(sysfs_cur_hw, p, i, v);
	} else	if (!strcmp(name, "tokens")) {
		mvPp2PlcrHwTokenValue(sysfs_cur_hw, p, i);
	} else	if (!strcmp(name, "bucket")) {
		mvPp2PlcrHwBucketSizeSet(sysfs_cur_hw, p, i, v);
	} else	if (!strcmp(name, "cpu_v0_tr")) {
		mvPp2V0PlcrHwCpuThreshSet(sysfs_cur_hw, p, i);
	} else	if (!strcmp(name, "cpu_v1_tr")) {
		mvPp2V1PlcrHwCpuThreshSet(sysfs_cur_hw, p, i);
	} else	if (!strcmp(name, "hwf_v0_tr")) {
		mvPp2V0PlcrHwHwfThreshSet(sysfs_cur_hw, p, i);
	} else	if (!strcmp(name, "hwf_v1_tr")) {
		mvPp2V1PlcrHwHwfThreshSet(sysfs_cur_hw, p, i);
	} else	if (!strcmp(name, "rxq_tr")) {
		mvPp2PlcrHwRxqThreshSet(sysfs_cur_hw, p, i);
	} else	if (!strcmp(name, "txq_tr")) {
		mvPp2PlcrHwTxqThreshSet(sysfs_cur_hw, p, i);
	} else
		printk(KERN_ERR "%s: illegal operation <%s>\n", __func__, name);
