...
static struct mv_pp2x_pme_entry  mv_pp2_pme_e;
			...
			struct device_attribute *attr, char *buf)
{
	...
	if (!strcmp(name, "sw_dump")) {
		...
		mvPp2PmeHwRegs(sysfs_cur_hw);
	} else	if (!strcmp(name, "hw_i_dump")) {
		mvPp2PmeHwDump(sysfs_cur_hw, 0);
	} else if (!strcmp(name, "hw_i_dump_all")) {
		mvPp2PmeHwDump(sysfs_cur_hw, 1);
	} else if (!strcmp(name, "hw_i_inv")) {
		mvPp2PmeHwInvAll(sysfs_cur_hw);
	} else {
			...
			struct device_attribute *attr, const char *buf, size_t len)
{
	...
	if (!strcmp(name, "hw_i_write")) {
		err = mvPp2PmeHwWrite(sysfs_cur_hw, t, &mv_pp2_pme_e);
	} else if (!strcmp(name, "sw_clear")) {
		err = mvPp2PmeSwClear(&mv_pp2_pme_e);
	} else if (!strcmp(name, "hw_i_read")) {
		err = mvPp2PmeHwRead(sysfs_cur_hw, t, &mv_pp2_pme_e);
		...
		err = mvPp2PmeHwDataTblDump(sysfs_cur_hw, t);
	} else if (!strcmp(name, "hw_d_clear")) {
		err = mvPp2PmeHwDataTblClear(sysfs_cur_hw, t);
	} else if (!strcmp(name, "hw_d_read")) {
		err = mvPp2PmeHwDataTblRead(sysfs_cur_hw, t, i, &data);
		printk(KERN_INFO "Data%d table entry #%d: 0x%04x\n", t, i, data);
	} else if (!strcmp(name, "hw_d_write")) {
		data = (unsigned short)v;
		err = mvPp2PmeHwDataTblWrite(sysfs_cur_hw, t, i, data);
	} else if (!strcmp(name, "ttl_zero")) {
		err = mvPp2PmeTtlZeroSet(sysfs_cur_hw, t);
	} else if (!strcmp(name, "max_config")) {
		err = mvPp2PmeMaxConfig(sysfs_cur_hw, t, i, v);
				...
				struct device_attribute *attr, const char *buf, size_t len)
{
	...
	if (!strcmp(name, "vlan_etype"))
		mvPp2PmeVlanEtherTypeSet(sysfs_cur_hw, i, v);
	else if (!strcmp(name, "dsa_etype"))
		mvPp2PmeDsaDefaultSet(sysfs_cur_hw, i, v);
	else if (!strcmp(name, "pppoe_proto"))
		mvPp2PmePppoeProtoSet(sysfs_cur_hw, i, v);
	else
		printk(KERN_ERR "%s: illegal operation <%s>\n", __func__, attr->attr.name);
				...
				struct device_attribute *attr, const char *buf, size_t len)
{
	...
	if (!strcmp(name, "sw_word"))
		...
		mvPp2PmeVlanDefaultSet(sysfs_cur_hw, v);
	else if (!strcmp(name, "dsa_src_dev"))
		mvPp2PmeDsaSrcDevSet(sysfs_cur_hw, v);
	else if (!strcmp(name, "pppoe_etype"))
		mvPp2PmePppoeEtypeSet(sysfs_cur_hw, v);
	else if (!strcmp(name, "pppoe_len"))
		mvPp2PmePppoeLengthSet(sysfs_cur_hw, v);
	else
		printk(KERN_ERR "%s: illegal operation <%s>\n", __func__, attr->attr.name);
